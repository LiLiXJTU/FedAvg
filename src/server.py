...
class Server(object):
    ...
    def mp_update_selected_clients(self, selected_index):
        ...
        train_loss,optimizer = self.clients[selected_index].client_update()
    ...
    def average_model(self, sampled_client_indices, coefficients):
        ...
        #             temp += coefficients[client_idx] * self.clients[client_idx].model.state_dict()[key]
        #         self.model.state_dict()[key].data.copy_(temp)
        #         for client_idx in range(3):
        #             self.clients[client_idx].state_dict()[key].data.copy_(self.model.state_dict()[key])
        #
        ...
        print('local_epochs', self.local_epochs)
        for key in self.model.state_dict().keys():
            if 'norm' not in key:
                severmodel_state_dict = self.model.state_dict()[key].to(self.device)
                temp = torch.zeros_like(severmodel_state_dict, dtype=torch.float32)
                # print('temp', temp.device)
                # print('self.model.state_dict()',self.model.state_dict()[key].device)
                for client_idx in range(3):
                    # print('self.clients[client_idx].device',self.clients[client_idx].device)
                    # print('self.device', self.device)
                    # print('self.clients[client_idx].model.state_dict()[key].device', self.clients[client_idx].model.state_dict()[key].device)
                    state_dict = self.clients[client_idx].model.state_dict()[key].to(self.device)
                    # print('state_dict',state_dict.device)
                    # print('temp',temp.device)
                    temp += client_weights[client_idx] * state_dict
                self.model.state_dict()[key].data.copy_(temp)
                for client_idx in range(3):
                    self.clients[client_idx].model.state_dict()[key].data.copy_(self.model.state_dict()[key])
            else:
                print(key)
